struct Day9 : Day {
  ...
  func part1 (_ input :[String]) throws -> String { String(parse(input).map(next).reduce(0, +)) }

  func prev (_ seq :[Int]) -> Int {
    seq.allSatisfy { $0 == 0 } ? 0 : seq.first! - prev(seq.adjacentPairs().map { $1 - $0 })
  }
  func part2 (_ input :[String]) throws -> String { String(parse(input).map(prev).reduce(0, +)) }
}
